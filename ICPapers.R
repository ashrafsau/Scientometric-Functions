...
ICPapers<-function(data_,year_){
  #Input: author address vector and corresponding year vector
  #Output: ICP mry-mry matrix 
  library(stringr)  
  data_<-data_[data_!=""]  
  C1<-data_
  ...
  m<-matrix(data = 0,nrow = length(years),ncol = 2, dimnames = list(years,c("Total","No.of.ICPapers")))   
  for(y in years){
    show(y)
    y_ind<-which(year_==y)
    cn<-as.character(y)
    m[cn,"Total"]<-length(y_ind)
    for(j in y_ind){
      ind<-which(str_detect(string = C1[j],pattern = ignore.case(c.list)))    
      if(length(ind)>1){
        m[cn,"No.of.ICPapers"]<-m[cn,"No.of.ICPapers"]+1
      }
    } 
  }
  m
...
#m<-ICPapers(data_ = data$C1,year_ = data$PY)
